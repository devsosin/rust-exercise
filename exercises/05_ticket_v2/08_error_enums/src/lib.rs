...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        ...
        Err(TicketNewError::TitleNotEmpty) => panic!("Title cannot be empty"),
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleNotEmpty) // Err(("Title cannot be empty".to_string()));
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleTooLong) // Err("Title cannot be longer than 50 bytes".to_string());
        }
